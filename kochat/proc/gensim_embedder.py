...
class GensimEmbedder(BaseProcessor):
    ...
    def similar(self,entity,text):
        loclist=['작업','출장','출근','퇴근']
        acclist=['추락','협착','충돌','깔림','화재','교통사고','미끄러짐']
        injurylist=['골절','타박상','절단','화상','사망']

        self._load_model()
        dicts={}
        if 'LOC' in entity:
            for h,loc in zip([self.model.wv.similarity(text,i) for i in loclist],loclist):
                dicts[h]=loc
        elif 'ACC' in entity:
            for h,acc in zip([self.model.wv.similarity(text,i) for i in acclist],acclist):
                dicts[h]=acc
        elif 'INJURY' in entity:
            for h,injury in zip([self.model.wv.similarity(text,i) for i in injurylist],injurylist):
                dicts[h]=injury

        print("embedicts",text,dicts)
